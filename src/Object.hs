...
data Color = Color (Triple Double)
...
data Object = Object { _color      :: Color
                     , _emittance  :: Double
                     , _reflective :: Bool
                     , _form       :: Form
                     , _name       :: String }
...
data Point = Point (Triple Double)
...
infLight = Object 
  ...
  , _reflective = True
  , _form       = InfinitePlane
             { _point  = Point $ Triple 0 0 (10)
             , _normal = Vector $ Triple 1 0 (-1) }
  }
...
disk2 = Object
  ...
  , _reflective  = False
  , _form        = Disk
    { _center = newPoint 0 0 10
    , _normal = newVector 1 0 (-1)
    , _radius = 600 }
  }
...
march (Ray (Point origin) (Vector vector)) distance = origin + fmap (distance *) vector
...
distanceFrom' ray@(Ray (Point origin) (Vector vector)) form =
  case form of
    Disk (Point center) (Vector normal) radius -> do
      distanceFromOrigin    <- distanceFrom ray $ InfinitePlane (Point center) (Vector normal)
...
getNormal form = normal
  where Vector normal = _normal form
